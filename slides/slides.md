...
<!--
- 🗣 "We're going to put a web page on the internet in the next five minutes. Then we're going to spend the rest of the hour building the tool that should have done it. By the end you'll know exactly how Terraform, Bicep and Pulumi work, because you'll have watched one get built."
...
<!--
- 🗣 "No SDK, no framework, just `import urllib`. A bearer token and four PUTs, top to bottom."
...
- → If pressed: Terraform's azurerm and Pulumi's azure-native default to Azure CLI auth locally too. Delegating "who am I" to az is what the real tools do. Removing it = a ~6-line client-credentials POST + a service principal on stage: worse demo, same lesson. And auth is the first per-cloud knowledge we outsource, which foreshadows the cliff (AWS SigV4, III_24).
...
- → This is migrations-for-infrastructure: an append-only ledger of every decision ever reversed. The fix is a picture of the goal, plus something that remembers what it did last time.
...
- 🗣 (click 2) "So we write it down ourselves, at the only moment we know it for sure: creation. The name I used, the ID Azure gave back. That table is state. It's just remembering your own actions."
...
- → (Q&A pocket, not a promise: `plan` is really a THREE-way merge, not a two-way diff, the same reason kubectl invented last-applied-configuration.)
-->
...
- → (apply clicks) Deletes first, then creates+updates. Create and update are the same call, since PUT is an upsert (the luck from the aside, now load-bearing). `plan` is just `diff` without `apply`.
...
- 🗣 (click 3) Savor the error: a blob PUT against a storage account that doesn't exist, and what our code reports is NoneType is not subscriptable. The cloud said 404. The traceback names nothing. This is what shipping the luck feels like.
...
Even so, the fix is **writing the order down**, not a stricter language.
...
- 🗣 (last click) "Still, the fix is writing the order down, not a stricter language. The script always knew the order, and the file threw it away."
...
- 🗣 (click 1) "The tidied file was this order. hello first, and its parents don't exist. Crash."
...
- 🗣 (the page says ~165, so head off the "wasn't it 114?" murmur) "Step 3 was 114 lines. The graph and the poller make it 132, and by the end of the hour it's about 180. That growth is the story of the rest of the talk."
...
- → Real measurements. Say the numbers out loud, and if today's run drifted, say so.
-->
...
<!--
- 🗣 "We sent seven. Azure answered with forty-eight. An honest diff wants to 'fix' forty-two things you never mentioned. Some are read-only and would 400."
...
<!--
- → Even a field we _own_ comes back reshaped. An allowlist of names isn't enough, compare shape-wise.
...
- 🗣 "Somebody, say a well-meaning intern in the portal, changes a tag. Not through our tool."
...
- → The ONE Pulumi slide (2026-09-01: III_24's provider line neutralized, C# slide hidden). Anything else you'd want to add, like generated providers or other languages, say it here or in Q&A. Don't slide it.
