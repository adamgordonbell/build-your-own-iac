...
def blob_sas(res):  # ask the management plane to sign us into the data plane
    ...
    return out["serviceSasToken"] if out else None   # no account, no signature
...
def blob_call(method, res, body=None):  # same verbs, different world
    ...
    if (sas := blob_sas(res)) is None:   # the account is gone — so is the blob
        return 404, None
    req = urllib.request.Request(f"{url}?{sas}", method=method,
...
def refresh(state):
    for key in list(state):
        res = state[key]["res"]
        if res["type"] == "blob":       # no envelope out here: GET returns the file itself
            code, content = blob_call("GET", res)
            if code == 404:
                print(f"  ! {key} vanished — deleted outside of us")
                del state[key]
                continue
            was, now = state[key]["saved"], {
                "contentType": res["properties"]["contentType"], "content": content}
...
def main():
    ...
    # two people, one state file: O_EXCL is atomic — "check then create" would
    # be a race of its own
    try:
